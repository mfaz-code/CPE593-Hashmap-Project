  ...
  public:
    ...
    void addWord(string word){
      ...
      for (int i = 0; i<word.length(); i++){ // iterating through every letter, from zero to length of word.
        char c = word[i] - 'a';  //setting char so that the 'c' variable gets assigned a value anywhere from 0-25: the 26 letters of the alphabet.
        if(p->next[c] == nullptr){
...
int main(){
 ...
 // while (f){ // while fstream 'f' is good and readable; // use this loop as the one that intakes and maps words to trie.
  ...
  auto endMapping = chrono::high_resolution_clock::now();
  chrono::duration<double> duration = endMapping - start; 

  f.close();
  cout << "dictionary file 'dict.txt' has been mapped. Time duration to map: " << duration.count() << endl;

  //cout << "please insert the file name (with .txt ending) which you want to check in the dictionary." << endl;
  //cin >> s;
  //fstream g(s);

  word = "ichthyopaleontology";
  //while (g){
    ...
    duration = endSearch - start;
    cout << "word search for " << word << ": duration: " << duration.count() << endl;
    cout << "size of dict (Mbytes): " << (dict.nodeCount*dict.nodeSize)/1024/1024;
